        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route('admin.customer.index') }}"
                    class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Customer Index</a>
            </div>
                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.customer.update', $customer->id) }}"
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="no_telp" name="no_telp" value="{{ $customer->no_telp }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('no_telp')
                                <div class="text-sm text-red-400">{{ $message }}</div>
