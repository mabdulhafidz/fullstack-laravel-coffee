<div wire:ignore.self>
        ...
        <div class="flex h-screen bg-gray-900">
            <div class="hidden md:flex flex-col items-center w-20 bg-gray-900 p-4">
            ...
            <div class="flex">
                ...
                <div class="flex-grow bg-blue-gray-50 p-4 overflow-auto grid grid-cols-4 gap-4">
                    @foreach ($menus as $menu)
                        <div class="bg-white rounded-md p-4 flex flex-col items-center border border-gray-300">
                            ...
                            <div>
                                <button
                                    wire:click="addToCart('{{ $menu->id }}', '{{ $menu->name }}', {{ $menu->price }})"
            ...
            <div class="w-5/12 bg-gray-900 h-full bg-white pr-4 pl-2 py-4 overflow-auto">
                <div class="flex-1 w-full px-4 overflow-auto">
                    <div class="select-none h-auto w-full text-center pt-3 pb-4 px-4">
                        <div class="flex flex-col space-y-4 text-lg font-semibold text-blue-gray-700">
                            @foreach ($cart as $item)
                                <div class="flex items-center justify-between border-b-2 border-gray-300 py-2">
                                    <div class="flex items-center space-x-2">
                                        <p class="text-white">Name: {{ $item['name'] }}</p>
                                        <p class="text-white">Price: ${{ $item['price'] * $item['qty'] }}</p>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <button wire:click="updateCart('{{ $item['id'] }}', {{ $item['qty'] - 1 }})"
                                            class="p-2 rounded-full text-white bg-red-500 hover:bg-red-600">
                                            -
                                        </button>
                                        <p class="text-white">{{ $item['qty'] }}</p>
                                        <button wire:click="updateCart('{{ $item['id'] }}', {{ $item['qty'] + 1 }})"
                                            class="p-2 rounded-full text-white bg-blue-500 hover:bg-blue-600">
                                            +
                                        </button>
                ...
                <div class="flex flex-col space-y-2 text-lg font-semibold text-blue-gray-700">
                ...
                <div class="mt-auto">
                    <div class="flex items-center justify-between">
                        ...
                        <!-- Include your payment method options here -->
                    </div>
