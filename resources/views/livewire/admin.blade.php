<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <div class="bg-slate-50 p-5 m-2 rounded-md flex justify-between items-center shadow">
            ...
            <svg class="h-8 w-8 text-pink-500"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <circle cx="9" cy="21" r="1" />  <circle cx="20" cy="21" r="1" />  <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6" /></svg>
    ...
    <div class="grid grid-cols-1 gap-2 lg:grid-cols-2">
        <!-- chart  -->
        <div id="revenue-chart" style="width: 100%; height: 400px;" wire:ignore>
            <canvas></canvas>
        </div>
        <div class="grid grid-cols-1">
            ...
            <div class="m-2 shadow-md">
                <div>
                    <h2 class="mb-4">Pendapatan Harian dan Persentase</h2>
                    <table>
                        <thead>
                            <tr>
                                <th class="p-2">Tanggal</th>
                                <th class="p-2">Pendapatan (USD)</th>
        ...
        <div class="overflow-x-auto m-2 shadow-md">
            <table class="w-full">
                <thead class="bg-pink-500 rounded-sm">
                    <tr>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <!-- chart -->
        <div class="m-2 lg:col-span-1 shadow-md overflow-y-auto">
            <header class="bg-pink-500 rounded-sm">
                <h2 class="font-bold mb-4 text-xl">Top 5 Penjualan</h2>
            </header>
            @forelse($mostOrderedMenus as $menu)
                <div class="mb-4 flex items-center">
                    <img src="{{ Storage::url($menu['menu']->image) }}" class="w-16 h-16 object-cover rounded-lg mr-4">
                    <div>
                        <p class="font-bold text-lg mb-1">{{ $menu['menu']->name }}</p>
                        <p>Terjual: {{ $menu['quantity'] }}</p>
                    </div>
                    <div class="ml-auto">
                        <p class="font-bold">${{ $menu['menu']->price }}</p>
                    </div>
                </div>
                <hr class="mt-2 mb-2">
            @empty
        ...
        <!-- candle list -->
        <div class="m-2 lg:col-span-2 shadow-md overflow-y-auto">
            <header class="bg-pink-500">
                <h2 class="font-bold mb-4 text-center">Sisa Stok Terendah</h2>
            </header>
            @if($lowStockItems->isNotEmpty())
                <table class="w-full">
                    <thead class="rounded-sm">
                        <tr class="mt-2 mb-2">
                            <th class="px-4 py-2">Menu Image</th>
                    ...
                    <tbody>
                        @foreach($lowStockItems as $stock)
                            <tr class="mb-4">
                                <td class="px-4 py-2">
                                    <img src="{{ Storage::url($stock->menu->image) }}" alt="Menu Image" class="w-16 h-16 object-cover rounded-lg">
                                </td>
                                <td class="px-4 py-2">{{ $stock->menu->name }}</td>
                                <td class="px-4 py-2 text-center">{{ $stock->jumlah }}</td>
                            </tr>                        
                        @endforeach
                    </tbody>
                </table>
